  ...
  const { data, isLoading, error } = useQuery<{ companies: Company[], total: number, hasMore: boolean }>({
    ...
    queryFn: async ({ queryKey }) => {
      ...
      const mapRow = (row: any): Company => ({
        ...
        name: row.name,
        symbol: row.symbol,
        marketCap: row.marketCap ?? row.market_cap ?? 0,
        price: row.price ?? 0,
        dailyChange: row.dailyChange ?? row.daily_change ?? null,
        dailyChangePercent: row.dailyChangePercent ?? row.daily_change_percent ?? null,
        ...
        peRatio: row.peRatio ?? row.pe_ratio ?? null,
        eps: row.eps ?? null,
        beta: row.beta ?? null,
        dividendYield: row.dividendYield ?? row.dividend_yield ?? null,
        priceToSalesRatio: row.priceToSalesRatio ?? row.price_to_sales_ratio ?? null,
        netProfitMargin: row.netProfitMargin ?? row.net_profit_margin ?? null,
        revenueGrowth3Y: row.revenueGrowth3Y ?? row.revenue_growth_3y ?? null,
        revenueGrowth5Y: row.revenueGrowth5Y ?? row.revenue_growth_5y ?? null,
        revenueGrowth10Y: row.revenueGrowth10Y ?? row.revenue_growth_10y ?? null,
        ...
        operatingIncome: row.operatingIncome ?? row.operating_income ?? null,
        netIncome: row.netIncome ?? row.net_income ?? null,
        totalAssets: row.totalAssets ?? row.total_assets ?? null,
        totalDebt: row.totalDebt ?? row.total_debt ?? null,
        cashAndEquivalents: row.cashAndEquivalents ?? row.cash_and_equivalents ?? null,
        return3Year: row.return3Year ?? row.return_3_year ?? null,
        return5Year: row.return5Year ?? row.return_5_year ?? null,
        return10Year: row.return10Year ?? row.return_10_year ?? null,
        maxDrawdown10Year: row.maxDrawdown10Year ?? row.max_drawdown_10_year ?? null,
        maxDrawdown5Year: row.maxDrawdown5Year ?? row.max_drawdown_5_year ?? null,
        maxDrawdown3Year: row.maxDrawdown3Year ?? row.max_drawdown_3_year ?? null,
        arMddRatio10Year: row.arMddRatio10Year ?? row.ar_mdd_ratio_10_year ?? null,
        arMddRatio5Year: row.arMddRatio5Year ?? row.ar_mdd_ratio_5_year ?? null,
        arMddRatio3Year: row.arMddRatio3Year ?? row.ar_mdd_ratio_3_year ?? null,
        freeCashFlow: row.freeCashFlow ?? row.free_cash_flow ?? null,
        returnDrawdownRatio10Year: row.returnDrawdownRatio10Year ?? row.return_drawdown_ratio_10_year ?? null,
        dcfEnterpriseValue: row.dcfEnterpriseValue ?? row.dcf_enterprise_value ?? null,
        marginOfSafety: row.marginOfSafety ?? row.margin_of_safety ?? null,
        dcfImpliedGrowth: row.dcfImpliedGrowth ?? row.dcf_implied_growth ?? null,
        totalEquity: row.totalEquity ?? row.total_equity ?? null,
        roe: row.roe ?? null,
        assetTurnover: row.assetTurnover ?? null,
        financialLeverage: row.financialLeverage ?? null,
      });
      return {
        ...raw,
