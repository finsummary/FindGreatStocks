...
/**
 * Script to populate data for newly added NASDAQ 100 companies
...
const SYMBOLS = ['ALNY', 'FER', 'INSM', 'MPWR', 'STX', 'WDC'];
  ...
  try {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        free_cash_flow: latestFcf ? latestFcf.toString() : null,
...
async function populateROIC10YHistory(symbol: string) {
  ...
  try {
    ...
    // Calculate ROIC for available years (up to 10)
    const maxYears = Math.min(10, Math.min((incomeStatementData?.length || 0), (balanceSheetData?.length || 0)));
    
    if (maxYears === 0) {
      console.log(`⚠️ No data available for ROIC history calculation for ${symbol}`);
      // Still update with nulls to ensure columns exist
      const updates = {
        roic_y1: null, roic_y2: null, roic_y3: null, roic_y4: null, roic_y5: null,
        roic_y6: null, roic_y7: null, roic_y8: null, roic_y9: null, roic_y10: null,
        roic_10y_avg: null, roic_10y_std: null, roic_stability: null, roic_stability_score: null,
      };
      await supabase.from('nasdaq100_companies').update(updates).eq('symbol', symbol);
      return true; // Return true so process continues
    }
    ...
    for (let i = 0; i < maxYears; i++) {
      ...
      if (investedCapital > 0 && !isNaN(nopat) && isFinite(nopat) && ebit !== 0) {
...
async function populateCurrentFcfMargin(symbol: string) {
  ...
  try {
    ...
    console.log(`✅ Updated FCF margin for ${symbol}: ${fcfMargin !== null && isFinite(fcfMargin) ? (fcfMargin * 100).toFixed(2) + '%' : 'N/A'}`);
    return true;
  } catch (error) {
...
async function main() {
  console.log('🚀 Starting data population for new NASDAQ 100 companies: ALNY, FER, INSM, MPWR, STX, WDC\n');
  ...
  for (const symbol of SYMBOLS) {
    ...
    if (companyData && companyData.market_cap) {
      ...
      if (marketCap && marketCap > 0) {
        await updateDcfMetricsForCompany(schema.nasdaq100Companies, symbol, marketCap);
...
export { main as populateNewNasdaq100Companies };

// Run if executed directly
if (import.meta.url === `file://${process.argv[1]}` || import.meta.url.endsWith('populate-new-nasdaq100-companies.ts')) {
