/**
 * FTSE 100 Daily Price and Market Cap Updater
 * Automatically updates stock prices and market caps for all FTSE 100 companies
...
export async function updateFTSE100Prices() {
  console.log('🚀 Starting FTSE 100 daily price update...');
  const startTime = Date.now();
  
  try {
    // Get all FTSE 100 companies
    const companies = await db.select().from(ftse100Companies);
    console.log(`Found ${companies.length} FTSE 100 companies to update`);
    ...
    for (const company of companies) {
      ...
      // Rate limiting - wait 150ms between requests (London market data)
    ...
    console.log(`✅ FTSE 100 price update completed in ${duration}s`);
    ...
    console.error('❌ FTSE 100 price update failed:', error);
    return {
      success: false,
...
import.meta.url === `file://${process.argv[1]}` && 
  updateFTSE100Prices().then(result => {
